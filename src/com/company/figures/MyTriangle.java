...
public class MyTriangle {
    private static final double FLT_EPSILON = 0.00001;
    private MyPoint v1;
    private MyPoint v2;
    private MyPoint v3;
    ...
    public String getType () {
        ...
        final double threshold = FLT_EPSILON;

        boolean triangleReal = (side1+side2) <= side3 || (side1+side3) <= side2 || (side2+side3) <= side1;

        if ( (Math.abs((side1 - side2)) < threshold) && (Math.abs((side2 - side3)) < threshold) && (Math.abs((side2 - side3)) < threshold) ) {
